...
import * as dev from 'request-promise'
...
export default async function ettvCentral(query = '', page = 0) {
  const ALLURLARRAY: string[] = []
  var ALLTORRENT: TorrentData[] = []
  const url =
    'https://www.ettvcentral.com/torrents-search.php?search= ' +
    query +
    '&page=' +
    page
  let html
  ...
  $('table tbody').each((_, element) => {
    $('tr').each((_, el) => {
      ...
      data.Url =
        'https://www.ettvcentral.com' + $(td).eq(1).find('a').attr('href')
      if (data.Name !== '') {
